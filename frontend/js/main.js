...
async function renderAnimeList(username) {
    try {
        ...
        const animeList = responseDto.list;
        ...
        for (const i in animeList) {
            ...
            overlay.appendChild(buildMalInfo(anime));
            item.appendChild(overlay);

            const episodes = anime.episodes;
            if (episodes.list.length > 0) {
                const dialog = buildDialog(episodes);
                item.appendChild(dialog);

                overlay.appendChild(buildSitesInfo(episodes, dialog));
            }
        }
...
function buildPoster(anime) {
    ...
    result.setAttribute('alt', anime.name);
    withTitle(result, anime.name);
    result.setAttribute('draggable', 'false');
    return result;
}

function buildMalInfo(anime) {
    const result = div('mal_info');

    if (anime.name) {
        result.appendChild(buildBadge(anime.name, anime.name));
    }

    result.appendChild(buildMalEpisode(anime));

    if (anime.airing) {
        result.appendChild(buildBadge('Airing', 'Airing'));
    }

    result.appendChild(buildMalLink(anime))
    return result;
}
...
function buildMalLink(anime) {
    const result = link('badge', anime.malUrl);
...
function buildDialog(episodes) {
    ...
    addTypeFilter(episodes, result);
    result.append(buildSites(episodes.list));
    return result;
}

function buildSitesInfo(episodes, dialog) {
    const result = div('sites_info');

    addTranslationTypeInfo(episodes, TYPE_DUB, result);
    addTranslationTypeInfo(episodes, TYPE_SUB, result);
...
function addTranslationTypeInfo(episodes, translationType, sitesInfo) {
    const matchedAmount = episodes[translationType];
    if (matchedAmount > 0) {
        let badge;
        let title;
...
function addTypeFilter(episodes, dialog) {
    const dubPresents = episodes.dub > 0;
    const subPresents = episodes.sub > 0;

    if (episodes.list.length > 1 && dubPresents && subPresents) {
