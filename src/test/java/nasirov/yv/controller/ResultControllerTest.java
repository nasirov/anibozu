...
public class ResultControllerTest extends AbstractTest {
	...
	public void checkResultForNewUser() throws Exception {
		...
		checkFront(content,
				fairyTail1.getFinalUrl(),
				fairyTail1.getPosterUrl(),
				fairyTail1.getTitleOnMAL(),
				fairyTail1.getEpisodeNumberForWatch(),
				sao.getPosterUrl(),
				sao.getTitleOnMAL(),
				notFoundAnime.getAnimeUrl(),
				notFoundAnime.getPosterUrl(),
				notFoundAnime.getTitle(),
				USERNAME);
	}
	...
	public void checkResultForCachedUser() throws Exception {
		...
		AnimediaMALTitleReferences sao1Updated = new AnimediaMALTitleReferences("anime/mastera-mecha-onlayn",
				"1",
				"1",
				"sword art online",
				"1",
		...
		userMALCache.put(USERNAME, watchingTitles);
		...
		matchedReferencesCache.put(USERNAME, new LinkedHashSet<>());
		doReturn(new ArrayList<>()).when(animediaService).getCurrentlyUpdatedTitles();
	...
	private void checkFront(String content, String availableFinalUrl, String availablePosterUrl, String availableTitle,
			String availableEpisodeNumberForWatch, String notAvailablePosterUrl, String notAvailableTitle, String notFoundMALUrl, String notFoundPosterUrl,
			String notFoundTitle, String username) {
		...
		pattern = Pattern.compile(
				"<p class=\"title\">New Episode Available</p>\\R\\s*<ul>\\R\\s*<a href=\"" + availableFinalUrl + "\" target=\"_blank\"><img src=\""
						+ availablePosterUrl + "\" height=\"318\" width=\"225\"\\R\\s+alt=\"" + availableTitle + "\"\\R\\s+title=\"" + availableTitle
						+ " episode " + availableEpisodeNumberForWatch + "\"");
		matcher = pattern.matcher(content);
		assertTrue(matcher.find());
		pattern = Pattern.compile("<p class=\"title\">New Episode Not Available</p>\\R\\s*<ul>\\R\\s*<img src=\"" + notAvailablePosterUrl
				+ "\" height=\"318\" width=\"225\" alt=\"" + notAvailableTitle + "\"\\R\\s+title=\"" + notAvailableTitle + "\" class=\"fade\"/>");
		matcher = pattern.matcher(content);
		assertTrue(matcher.find());
		pattern = Pattern.compile(
				"<p class=\"title\">Not Found on Animedia</p>\\R\\s*<ul>\\R\\s*<a href=\"" + notFoundMALUrl + "\" target=\"_blank\"><img src=\""
						+ notFoundPosterUrl + "\" height=\"318\" width=\"225\"\\R\\s+alt=\"" + notFoundTitle + "\"\\R\\s+title=\"" + notFoundTitle
						+ "\" class=\"fade\"/></a>");
		matcher = pattern.matcher(content);
		assertTrue(matcher.find());
	}
