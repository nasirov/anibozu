...
class UserApiTest extends AbstractTest {
	...
	@Test
	void shouldReturnAnimeListFandubDataIsEmpty() {
		//given
		stubAnimeListOk();
		//when
		ResponseSpec result = getAnimeList();
		//then
		checkResponse(result, "expected_anime_list_empty_fandub_info_list.json");
	}
	...
	void shouldReturnErrorFandubDataServiceException() {
		//given
		stubAnimeListOk();
		doThrow(new RuntimeException("FandubDataService cause")).when(fandubDataService).getFandubData(any(FandubDataId.class));
		//when
		ResponseSpec result = getAnimeList();
		//then
		checkGenericErrorResponse(result);
