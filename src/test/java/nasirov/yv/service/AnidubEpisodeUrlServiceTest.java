...
public class AnidubEpisodeUrlServiceTest extends AbstractTest {
	...
	public void setUp() {
		super.setUp();
		mockGithubResourcesService(buildTitles());
	}
	...
	public void shouldFindAvailableUrl() {
		...
		UserMALTitleInfo title = buildWatchingTitle(REGULAR_TITLE_MAL_ID, REGULAR_TITLE_NAME, 0, REGULAR_TITLE_POSTER_URL, REGULAR_TITLE_MAL_ANIME_URL);
		//when
	...
	public void shouldFindNotAvailableUrl() {
		//given
		mockAnidub("anidub/titleEpisodes.json");
		UserMALTitleInfo title = buildWatchingTitle(REGULAR_TITLE_MAL_ID, REGULAR_TITLE_NAME, 3, REGULAR_TITLE_POSTER_URL, REGULAR_TITLE_MAL_ANIME_URL);
		//when
		String actualUrl = anidubEpisodeUrlService.getEpisodeUrl(title);
		//then
	...
	public void shouldIgnoreEpisodesWithInvalidHosts() {
		...
		UserMALTitleInfo title = buildWatchingTitle(REGULAR_TITLE_MAL_ID, REGULAR_TITLE_NAME, 1, REGULAR_TITLE_POSTER_URL, REGULAR_TITLE_MAL_ANIME_URL);
		//when
		String actualUrl = anidubEpisodeUrlService.getEpisodeUrl(title);
		//then
		assertEquals(FINAL_URL_VALUE_IF_EPISODE_IS_NOT_AVAILABLE, actualUrl);
	...
	private Set<AnidubTitle> buildTitles() {
		return Sets.newHashSet(buildRegularAnidubTitle());
	}

	private AnidubTitle buildRegularAnidubTitle() {
