package nasirov.yv.http.feign.fandub.animedia;

import java.util.List;
import nasirov.yv.data.fandub.animedia.AnimediaEpisode;
import nasirov.yv.http.config.FeignClientConfig;
import nasirov.yv.http.fallback.fandub.animedia.AnimediaFeignClientFallbackFactory;
 ...
 */
@FeignClient(value = "animedia-feign-client", configuration = FeignClientConfig.class, fallbackFactory = AnimediaFeignClientFallbackFactory.class)
...
public interface AnimediaFeignClient {
	...
	List<AnimediaEpisode> getEpisodes(@PathVariable("animeId") String animeId, @PathVariable("dataList") String dataList);
}